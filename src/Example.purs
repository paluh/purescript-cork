...
import Cork.Sprites.Sprite (clip, externalImage, fromImageBitmap)
import Cork.Sprites.Sprite (grayscale, grayscaleToAlpha, stackedBlur) as Sprite
import Cork.Svg.Path (Command(..)) as Path
...
import Geometry.Plane.BoundingBox (unsafe) as BoundingBox
...
main :: Effect Unit
main = do
  window >>= document >>= getHTMLElementById "app"
    >>= case _ of
        Just parent → do
          let
            dimensions =
            ...
            quad1 =
              ...
              , third: point 200.0 300.0
              , fourth: point 0.0 200.0
              }

            clip1 =
              clip
                ( List.Cons (Path.MoveAbs quad1.first)
                    $ List.Cons (Path.LineAbs quad1.second)
                    $ List.Cons (Path.LineAbs quad1.third)
                    $ List.Cons (Path.LineAbs quad1.fourth)
            ...
            workspace =
              ...
              , drawImageData p7 defaultStyle clip1
              ]
