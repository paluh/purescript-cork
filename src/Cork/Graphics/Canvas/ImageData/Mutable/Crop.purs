module Cork.Graphics.Canvas.ImageData.Mutable.Crop
  ( crop
  )
  where
...
crop ∷ Maybe CanvasElement → BoundingBox Units.Pixel → Mutable → Aff Mutable
crop possibleCanvas boundingBox orig = do
  let
    d = dimensions orig
  canvas ← liftEffect $ maybe (CanvasElement.new' d) pure  possibleCanvas
