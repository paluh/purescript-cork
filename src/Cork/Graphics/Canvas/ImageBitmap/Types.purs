...
import Web.DOM.Document (toNode) as Document
...
-- | XXX: I WAS FORCED BY FIREFOX TO CURRENTLY NOT USE REAL `ImageBitmap`
-- |      We can think of efficient functionallity test
-- |      - it seems that simple clipping introduces black edges
-- |      when rendered into `ImageBitmap` and this can be used as test.
-- |      `premultiplyAlpha` doesn't change anything.
-- |
selfUnsafeCreateImageBitmap ∷ ∀ img. Effect (Maybe (img → Aff ImageBitmap))
selfUnsafeCreateImageBitmap = pure Nothing
  -- selfUnsafeCreateImageBitmapImpl >>= toMaybe >>> case _ of
