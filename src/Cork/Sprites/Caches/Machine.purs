...
type Draw
  = DrawF Sprite.ImageData Sprite.ImageBitmap
...
type DrawingPlanBase m imageData imageBitmap
  = Plan Hash m (Item imageData) (Item imageBitmap)
...
unfoldDrawPlan caches interpreters (DrawF d) = case d of
  ...
  where
  unfoldImageBitmapPlan' imageBitmap = case Map.lookup (hash imageBitmap) caches.imageBitmap of
    Just item → mempty
    Nothing → interpreters.imageBitmap imageBitmap (const $ mempty)

  unfoldImageDataPlan' imageData@(In i) = case Map.lookup (hash i) caches.imageData of
    Just item → mempty
    Nothing → interpreters.imageData imageData (const $ mempty)

type DrawingPlan
  = DrawingPlanBase Aff Canvas.ImageData Canvas.ImageBitmap
...
unfoldImageBitmapPlan workspace imageBitmap build = case imageBitmap of
  Sprite.ExternalImage hash url →
    ...
    ib = do
      ...
      pure r
  Sprite.FromImageData hash imageData →
    unfoldImageDataPlan workspace (roll imageData) \i →
      let
        process = do
          Aff.bracket
      ...
      in
        Plan empty (Map.singleton hash (Tuple process build))
...
unfoldImageDataPlan workspace imageData build =
  let
    x = do
      ...
      Nothing
  in
    case unroll imageData of
      Sprite.Blur hash b imageData →
        unfoldImageDataPlan workspace imageData \i →
          let
            process i' =
              Aff.bracket
                (AVar.Aff.take workspace)
                (flip AVar.Aff.put workspace) \c1 →
                liftEffect
                  $ do
      ...
      Sprite.ClipImageBitmap hash path edgeStyle bb imageBitmap →
        unfoldImageBitmapPlan workspace imageBitmap \i →
          let
            process i' =
              Aff.bracket
                (AVar.Aff.take workspace)
                (flip AVar.Aff.put workspace) \w →
                liftEffect
                  $ do
          ...
          in
            step hash process build i
      Sprite.FromImageBitmap hash bb imageBitmap →
        unfoldImageBitmapPlan workspace imageBitmap \i →
          let
            process i' = do
          ...
          in
            step hash process build i
      Sprite.Grayscale hash mode imageData →
        unfoldImageDataPlan workspace imageData \i →
          let
          ...
          in
            step hash process build i
      Sprite.GrayscaleToAlpha hash mode imageData →
        unfoldImageDataPlan workspace imageData \i →
          let
          ...
          in
            step hash process build i
      Sprite.Project hash quad imageBitmap →
        unfoldImageBitmapPlan workspace imageBitmap \i →
          let
            process i' =
              Aff.bracket
          ...
          in
            step hash process build i
      Sprite.StackedBlur hash radius imageData →
        unfoldImageDataPlan workspace imageData \i →
          let
    ...
    (Right imageData) →
      let
        x = do
          ...
          Nothing
      in
        Plan (Map.singleton hash (Tuple (process imageData) cont)) empty
    (Left err) →
      let
        x = do
          traceM "Step error"
          ...
          Nothing
      in
        Plan (Map.singleton hash (Tuple (pure (Left err)) cont)) empty
