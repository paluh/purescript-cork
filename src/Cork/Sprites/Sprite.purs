...
instance foldableImageBitmapF ∷ Foldable ImageBitmapF where
  ...
  foldMap f (Project _ _ i) = foldMap f i
...
grayscaleToAlpha mode t@(In i) = roll $ GrayscaleToAlpha (Hashable.hash ("grayscaleToAlpha" /\ mode /\ i)) mode t

stackedBlur ∷ Int → ImageData → ImageData
stackedBlur radius t@(In i) = roll $ StackedBlur (Hashable.hash ("stackedBlur" /\ radius /\ i)) radius t
...
project ∷ Quadrilateral Units.Pixel → ImageBitmap → ImageBitmap
project quad i = Project (Hashable.hash ("project" /\ Cork.Hashable.quadrilateral quad /\ i)) quad i
